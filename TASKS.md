...
- [ ] **P2.0 — Ingestion service scaffolding (depends: P0.1).** Create `internal/services/ingestion/`, wire the `ingestion` client into `ProviderData` (region-routed — reuse `analyticsregion`), and add a shared `client.go` helper. Blocks P2.1a–e.
