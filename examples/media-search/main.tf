...
# A streaming catalog: movies and shows in one searchable "titles" index.
...
resource "algolia_index" "titles" {
  ...
  attributes {
    ...
    # Collapse every episode of a series into a single hit.
...
# Merchandising: when a query expresses browse/discovery intent, surface the
# platform's own originals.
...
# Domain synonyms so viewers find titles regardless of the term they use.
