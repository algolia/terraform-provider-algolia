...
func apiKeyDataSourceSchema() datasourceschema.Schema {
	return datasourceschema.Schema{
		Description: "Use this data source to read an existing Algolia API key's metadata by its key value.",
		Attributes: map[string]datasourceschema.Attribute{
			"id": datasourceschema.StringAttribute{
				Description: "The actual API key value (same as `key`).",
				Computed:    true,
				Sensitive:   true,
			},
