...
- Every user-visible change gets a `CHANGELOG.md` entry under `FEATURES:`,
  ...
  has to do differently, not just what changed. Since `0.1.0` is released company-wide,
  read the state-compatibility section below before breaking anything state-shaped: a
  released version means migration, not just a note.
...
**`0.1.0` is released and distributed company-wide.** It supersedes `v0.1.0-beta.1`, and
because it carries no pre-release marker, range constraints such as `~> 0.1` resolve
against it where they could never match a `-beta` build. State written by a released
build is therefore not only out there, it is out there across teams whose configurations
you cannot read.

That retires the reasoning that made breaking changes cheap. "Internal" bounded the
audience to a handful of reachable people; it no longer does. Treat a state-shaped change
as something to be migrated rather than announced.
